...
export default function PatientDashboard() {
  ...
  return (
    <div className="relative flex h-screen w-screen bg-[#07090e] text-slate-100 font-sans selection:bg-cyan-500/20 selection:text-cyan-300 lg:flex-row overflow-hidden">
      ...
      <aside
        className="group/sidebar sticky top-0 z-30 flex shrink-0 flex-col h-screen border-r border-blue-900/40 bg-gradient-to-b from-[#0a0f1d] via-[#0d162d] to-[#07090e] transform-gpu transition-[width] duration-300 ease-out will-change-[width] [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden shadow-2xl shadow-blue-950/40 lg:w-[58px] lg:hover:w-60"
      >
        {/* Brand Header */}
        <div className="flex h-14 shrink-0 items-center border-b border-blue-900/40 px-2.5">
          <div className="flex items-center gap-2.5 overflow-hidden w-full cursor-pointer" onClick={() => navigate("/")}>
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-500 via-blue-600 to-indigo-700 text-white shadow-[0_0_15px_rgba(6,182,212,0.4)]">
            ...
            <div className="transition-opacity duration-150 overflow-hidden whitespace-nowrap opacity-0 delay-0 duration-100 group-hover/sidebar:opacity-100 group-hover/sidebar:delay-75 group-hover/sidebar:duration-150">
              ...
              <p className="mt-0.5 text-[9px] font-mono uppercase tracking-widest text-cyan-400/90">Patient Portal</p>
            </div>
          </div>
        </div>
            ...
            return (
              <button
                ...
                className={`w-full h-9 rounded-xl flex items-center transition-all duration-200 ${
                  isActive
                    ? "bg-gradient-to-r from-cyan-500/25 to-blue-600/25 text-cyan-300 border border-cyan-400/40 shadow-[0_0_16px_rgba(6,182,212,0.2)] font-bold backdrop-blur-xs"
        ...
        {/* Bottom Toolbar */}
        <div className="border-t border-blue-900/40 p-2 flex flex-col gap-1 shrink-0">
          <button
            onClick={handleLogout}
            title="Logout"
            className="w-full h-9 rounded-xl flex items-center transition-colors duration-150 text-slate-400 hover:bg-rose-500/20 hover:text-rose-300 hover:border-rose-400/30 border border-transparent"
          >
            <span className="flex h-9 w-9 shrink-0 items-center justify-center">
