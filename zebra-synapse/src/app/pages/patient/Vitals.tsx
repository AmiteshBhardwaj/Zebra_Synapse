...
function statusBadgeClass(status: VitalsStatus) {
  if (status === "normal") return "border border-emerald-500/40 bg-emerald-950/40 text-emerald-300 font-bold";
  if (status === "elevated") return "border border-amber-500/40 bg-amber-950/40 text-amber-300 font-bold";
  return "border border-rose-500/40 bg-rose-950/40 text-rose-300 font-bold";
}
...
export default function Vitals() {
  ...
  return (
    <PatientPortalPage>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-blue-900/40 mb-6">
        <div className="flex items-center gap-3.5">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-600/20 border border-cyan-500/30 text-cyan-300 shadow-sm">
            <Activity className="h-6 w-6" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-white font-['Manrope']">Vitals & Body Metrics</h1>
              <span className="rounded-full border border-cyan-500/30 bg-cyan-950/40 px-2.5 py-0.5 text-[10px] font-bold text-cyan-300 uppercase tracking-wider">
                Real Profile
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="flex items-center gap-1.5 rounded-2xl border border-blue-900/40 bg-slate-900/80 px-3.5 py-1.5 text-slate-200 font-semibold shadow-sm">
      ...
      {loadError ? (
        <section className={`${portalPanelClass} border-rose-500/30 bg-rose-950/40 p-6 shadow-sm`}>
          <div className="flex items-start gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-rose-500/40 bg-rose-900/40">
              <AlertTriangle className="h-5 w-5 text-rose-400" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-white">Could not load vitals</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3.5 mb-8">
        <Card className={portalPanelClass}>
          <CardContent className="p-4">
            ...
            <p className="text-xl font-bold font-mono text-white truncate" title={formatHeight(patientHeight)}>
        ...
        <Card className={portalPanelClass}>
          <CardContent className="p-4">
            ...
            <p className="text-xl font-bold font-mono text-white truncate" title={formatWeight(patientWeight)}>
      ...
      {hasAnyVitals && summary ? (
        <>
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            ...
            <Card className={portalPanelClass}>
              <CardContent className="p-5">
                ...
                <p className="mt-3 text-xs text-slate-400 font-medium">Updated {summary.lastUpdated}</p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr] mt-6">
            <Card className={portalPanelClass}>
              <CardHeader>
                <CardTitle className="text-base font-bold text-white font-['Manrope']">
              ...
              <CardContent className="space-y-3">
                <div className="rounded-2xl border border-blue-900/40 bg-slate-950/60 p-3.5 sm:p-4">
                  <p className="text-[11px] uppercase font-bold tracking-wider text-cyan-400/80">Primary Condition</p>
                  <p className="mt-1 text-sm font-bold text-white">{summary.condition}</p>
                </div>
                <div className="rounded-2xl border border-blue-900/40 bg-slate-950/60 p-3.5 sm:p-4">
                  <p className="text-[11px] uppercase font-bold tracking-wider text-cyan-400/80">Last Updated</p>
                  <p className="mt-1 text-sm font-bold text-white">{summary.lastUpdated}</p>
                </div>
                <div className="rounded-2xl border border-blue-900/40 bg-slate-950/60 p-3.5 sm:p-4">
                  <p className="text-[11px] uppercase font-bold tracking-wider text-cyan-400/80">Source</p>
            ...
            <Card className={portalPanelClass}>
              ...
              <CardContent>
                {summary.source === "care" && summary.riskFlags.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {summary.riskFlags.map((flag) => (
                      <Badge
                        key={flag}
                        variant="outline"
                        className="border-blue-900/40 bg-slate-800/80 px-3 py-1 text-slate-200 font-semibold"
                      >
                        {flag}
                      </Badge>
                    ))}
                  ...
                  <div className="rounded-2xl border border-blue-900/40 bg-slate-950/60 p-4">
                  ...
                  <div className="rounded-2xl border border-blue-900/40 bg-slate-950/60 flex items-center gap-3 p-4">
          ...
          {(profile?.height_cm || profile?.weight_kg || profile?.dietary_preference) && (
            <Card className={`${portalPanelClass} mt-6`}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <CardTitle className="text-base font-bold text-white font-['Manrope']">Body Metrics & Configured Health Profile</CardTitle>
                  </div>
                  <Badge className="border border-cyan-500/30 bg-cyan-950/40 text-cyan-300 text-xs font-bold">
                    From Settings
                  </Badge>
                </div>
                <CardDescription className="text-xs text-slate-400">
              ...
              <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="rounded-2xl border border-blue-900/40 bg-slate-950/60 p-4">
                  <p className="text-[11px] uppercase tracking-wider text-cyan-400/80 font-bold">Height</p>
                ...
                <div className="rounded-2xl border border-blue-900/40 bg-slate-950/60 p-4">
                  <p className="text-[11px] uppercase tracking-wider text-cyan-400/80 font-bold">Weight</p>
                  <p className="mt-1 text-lg font-bold font-mono text-white">
                ...
                <div className="rounded-2xl border border-blue-900/40 bg-slate-950/60 p-4">
                  <p className="text-[11px] uppercase tracking-wider text-cyan-400/80 font-bold">BMI</p>
